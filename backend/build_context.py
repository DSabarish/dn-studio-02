...
try:
    ...
    try:
        ...
        try:
            module_path = Path(__file__).resolve().parent / "gemini_call.py"
            spec = importlib.util.spec_from_file_location("gemini_call_module", module_path)
            if spec is None or spec.loader is None:
                raise RuntimeError("Could not load gemini_call.py")
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            gemini_call = module.gemini_call
            _gemini_import_error = None
        except Exception as exc3:
            _gemini_import_error = exc3
            gemini_call = None
